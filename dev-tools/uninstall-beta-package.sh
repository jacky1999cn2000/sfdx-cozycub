...
echoStepMsg "Attempt to uninstall package $PACKAGE_VERSION_ID from the org aliased as $TEST_ORG_ALIAS"
echo \
...
if [ $? -ne 0 ]; then
  echoErrorMsg "Package removal from $TEST_ORG_ALIAS was not successful."
fi

#### 2. List all of the packages installed in the target subscriber org.
echoStepMsg "List all packages that remain in $TEST_ORG_ALIAS"
